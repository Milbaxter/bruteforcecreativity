"""
EUFN European Banks — iShares MSCI Europe Financials (EUFN) direction
as a global banking health signal.

European banks are more globally exposed than US banks. When EUFN rises,
global credit conditions are healthy. When EUFN falls, European banking
stress often precedes global contagion.
Combined with Wikipedia "European_debt_crisis" attention.
"""
...
STRATEGY = {
    "name": "EUFN European Banks",
    "hypothesis": "European banks are the weakest link in global banking. When EUFN rises, even the weakest banks are healthy = global credit conditions excellent = growth. When EUFN falls, European banking stress = potential global contagion = safety.",
    "universe": ["EUFN", "SOXX", "GDX", "SLV", "XLF"],
    "entry": "EUFN 7d momentum positive: buy SOXX/XLF. EUFN negative + wiki EU debt attention: buy GDX/SLV.",
    "exit": "Rotate every 5 trading days",
    "position_size": "100% in best 5d momentum candidate per regime",
    "eccentricity": "European bank health as a US sector rotation signal — most US investors don't monitor EUFN but European banking stress has historically preceded global equity corrections.",
}
...
def run(data_fetcher, portfolio, start_date, end_date):
    ...
    for ticker in STRATEGY["universe"]:
        try:
            ...
            continue
    if "EUFN" not in prices or len(prices) < 4:
        return
    try:
        wiki = data_fetcher.get_wikipedia_pageviews("European_debt_crisis")

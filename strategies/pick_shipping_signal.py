"""
BDRY Shipping Signal — Breakwave Dry Bulk Shipping ETF (BDRY) direction
as a global trade demand signal. Dry bulk shipping rates reflect real
goods moving across oceans = purest measure of physical global trade.
Combined with Wikipedia "Baltic_Exchange" attention.
"""
import pandas as pd
import numpy as np

STRATEGY = {
    "name": "BDRY Shipping Signal",
    "hypothesis": "Dry bulk shipping rates (via BDRY) are the purest measure of physical global trade. When shipping rates rise, goods are moving = demand is real. When rates fall, trade is slowing. No financial engineering can fake physical shipping demand.",
    "universe": ["BDRY", "SOXX", "GDX", "SLV", "XLF"],
    "entry": "BDRY 7d momentum positive: buy SOXX/XLF. BDRY negative: buy GDX/SLV.",
    "exit": "Rotate every 5 trading days",
    "position_size": "100% in best 5d momentum candidate per regime",
    "eccentricity": "Global shipping rates as a sector rotation timer — the Baltic Dry Index via BDRY captures actual physical goods movement. Most quants use PMI surveys; this is the real thing.",
}
...
def run(data_fetcher, portfolio, start_date, end_date):
    ...
    for ticker in STRATEGY["universe"]:
        try:
            ...
            continue
    if "BDRY" not in prices or len(prices) < 4:
        return
    try:
        wiki = data_fetcher.get_wikipedia_pageviews("Baltic_Exchange")

"""
CPER Copper Barometer — United States Copper Index Fund (CPER) direction
as "Dr. Copper" economic health signal.

Copper has a PhD in economics — it's used in construction, electronics,
EVs, and industrial manufacturing. When copper rises, real economic
activity is increasing. When copper falls, the economy is slowing.
Combined with Wikipedia "copper" attention.
"""
...
STRATEGY = {
    "name": "CPER Copper Barometer",
    "hypothesis": "Copper is the most economically-sensitive metal. Rising CPER = construction, manufacturing, EV production all healthy = growth. Falling CPER = industrial slowdown = safety. The copper ETF provides a cleaner signal than copper futures because it includes storage costs and contango effects.",
    "universe": ["CPER", "SOXX", "GDX", "SLV", "XLF"],
    "entry": "CPER 7d momentum positive: buy SOXX/XLF. CPER negative: buy GDX/SLV.",
    "exit": "Rotate every 5 trading days",
    "position_size": "100% in best 5d momentum candidate per regime",
    "eccentricity": "Dr. Copper as a sector rotation timer — copper's economic sensitivity is well known but rarely systematized into a daily rotation signal for non-commodity sectors.",
}
...
def run(data_fetcher, portfolio, start_date, end_date):
    ...
    for ticker in STRATEGY["universe"]:
        try:
            ...
            continue
    if "CPER" not in prices or len(prices) < 4:
        return

    try:
        wiki = data_fetcher.get_wikipedia_pageviews("Copper")

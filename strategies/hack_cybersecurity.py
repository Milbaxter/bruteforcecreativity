"""
HACK Cybersecurity — ETFMG Prime Cyber Security ETF (HACK) direction as
a tech enterprise spending signal. Cybersecurity spending is non-discretionary
for enterprises — when HACK rises, corporate IT budgets are expanding.
Combined with Wikipedia "cyberattack" attention as threat awareness filter.
"""
import pandas as pd
import numpy as np

STRATEGY = {
    "name": "HACK Cybersecurity Signal",
    "hypothesis": "Cybersecurity spending (HACK) is mandatory corporate spend. Rising HACK = enterprise IT budgets expanding = tech spending cycle bullish = growth. Falling HACK = budget cuts even in security = serious economic stress = safety.",
    "universe": ["HACK", "SOXX", "GDX", "SLV", "XLF"],
    "entry": "HACK 7d momentum positive + wiki cyberattack not spiking: buy SOXX/XLF. HACK negative: buy GDX/SLV.",
    "exit": "Rotate every 5 trading days",
    "position_size": "100% in best 5d momentum candidate per regime",
    "eccentricity": "Cybersecurity as enterprise spending barometer — if companies cut security budgets, they're cutting everything. HACK falling is a stronger recession signal than most indicators.",
}
...
def run(data_fetcher, portfolio, start_date, end_date):
    ...
    for ticker in STRATEGY["universe"]:
        try:
            ...
            continue
    if "HACK" not in prices or len(prices) < 4:
        return
    try:
        wiki = data_fetcher.get_wikipedia_pageviews("Cyberattack")

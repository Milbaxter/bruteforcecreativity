"""
LIT Lithium EV — Global X Lithium & Battery Tech ETF (LIT) direction as
an EV/clean energy demand signal.

Lithium is essential for batteries. Rising LIT = EV adoption accelerating
= tech/growth positive. Falling LIT = EV slowdown or oversupply = safety.
Combined with Wikipedia "electric_vehicle" attention.
"""
import pandas as pd
import numpy as np
...
STRATEGY = {
    "name": "LIT Lithium EV",
    "hypothesis": "Lithium battery demand (LIT) captures the EV transition momentum. Rising LIT = EV adoption accelerating = tech spending positive = growth. Falling LIT = EV demand softening or lithium oversupply = defensive.",
    "universe": ["LIT", "SOXX", "GDX", "SLV", "XLF"],
    "entry": "LIT 7d momentum positive: buy SOXX/XLF. LIT negative + EV wiki attention: buy GDX/SLV.",
    "exit": "Rotate every 5 trading days",
    "position_size": "100% in best 5d momentum candidate per regime",
    "eccentricity": "Lithium battery sector as a macro signal for tech/growth rotation — the EV supply chain reveals consumer and industrial spending patterns before traditional metrics.",
}
...
def run(data_fetcher, portfolio, start_date, end_date):
    ...
    for ticker in STRATEGY["universe"]:
        try:
            ...
            continue
    if "LIT" not in prices or len(prices) < 4:
        return
    try:
        wiki = data_fetcher.get_wikipedia_pageviews("Electric_vehicle")

"""
EWY South Korea Signal — iShares MSCI South Korea ETF (EWY) direction as
semiconductor supply chain signal. South Korea = Samsung + SK Hynix =
major DRAM/NAND supplier. EWY rising = memory chip demand healthy.
Combined with Wikipedia "Samsung_Electronics" attention.
"""
import pandas as pd
import numpy as np

STRATEGY = {
    "name": "EWY Korea Chips",
    "hypothesis": "South Korea's economy is dominated by Samsung and SK Hynix — the world's largest memory chip makers. Rising EWY = DRAM/NAND demand healthy = tech spending cycle bullish. Falling EWY = memory chip downturn = tech spending declining.",
    "universe": ["EWY", "SOXX", "GDX", "SLV", "XLF"],
    "entry": "EWY 7d momentum positive: buy SOXX/XLF. EWY negative: buy GDX/SLV.",
    "exit": "Rotate every 5 trading days",
    "position_size": "100% in best 5d momentum candidate per regime",
    "eccentricity": "South Korean equities as a semiconductor supply chain signal — Korea leads the memory chip cycle which is the foundation of AI/data center infrastructure.",
}
...
def run(data_fetcher, portfolio, start_date, end_date):
    ...
    for ticker in STRATEGY["universe"]:
        try:
            ...
            continue
    if "EWY" not in prices or len(prices) < 4:
        return
    try:
        wiki = data_fetcher.get_wikipedia_pageviews("Samsung_Electronics")

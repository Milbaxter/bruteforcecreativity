"""
IGV Software Signal — iShares Expanded Tech-Software Sector ETF (IGV)
direction as enterprise software spending signal. Rising IGV = enterprise
digital transformation = growth cycle. Combined with Wikipedia "SaaS" attention.
"""
import pandas as pd
import numpy as np

STRATEGY = {
    "name": "IGV Software Signal",
    "hypothesis": "Enterprise software (IGV) captures corporate IT spending. Rising IGV = companies investing in digital transformation = growth. Falling IGV = IT budget cuts = recession signal. Software is recurring revenue so IGV falling means serious demand destruction.",
    "universe": ["IGV", "SOXX", "GDX", "SLV", "XLF"],
    "entry": "IGV 7d momentum positive: buy SOXX/XLF. IGV negative: buy GDX/SLV.",
    "exit": "Rotate every 5 trading days",
    "position_size": "100% in best 5d momentum candidate per regime",
    "eccentricity": "Software spending as growth barometer — when Salesforce and Adobe slow, the rest of tech follows.",
}
...
def run(data_fetcher, portfolio, start_date, end_date):
    ...
    for ticker in STRATEGY["universe"]:
        try:
            ...
            continue
    if "IGV" not in prices or len(prices) < 4:
        return
    try:
        wiki = data_fetcher.get_wikipedia_pageviews("Software_as_a_service")

"""
SLX Steel Industry — VanEck Steel ETF (SLX) direction as an industrial
construction demand signal.

Steel demand rises with construction, infrastructure, and manufacturing.
When SLX rises, industrial activity is expanding = growth.
Combined with Wikipedia "steel_industry" attention.
"""
...
STRATEGY = {
    "name": "SLX Steel Industry",
    "hypothesis": "Steel is the backbone of construction and heavy industry. Rising SLX = infrastructure spending and manufacturing demand growing = broad economic expansion. Falling SLX = industrial slowdown = safety mode.",
    "universe": ["SLX", "SOXX", "GDX", "SLV", "XLF"],
    "entry": "SLX 7d momentum positive: buy SOXX/XLF. SLX negative: buy GDX/SLV.",
    "exit": "Rotate every 5 trading days",
    "position_size": "100% in best 5d momentum candidate per regime",
    "eccentricity": "Steel stocks as a macro rotation signal — steel demand is a direct measure of construction and infrastructure activity, more tangible than PMI surveys.",
}
...
def run(data_fetcher, portfolio, start_date, end_date):
    ...
    for ticker in STRATEGY["universe"]:
        try:
            ...
            continue
    if "SLX" not in prices or len(prices) < 4:
        return
    try:
        wiki = data_fetcher.get_wikipedia_pageviews("Steel_industry")

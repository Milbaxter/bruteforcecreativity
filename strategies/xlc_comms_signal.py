"""
XLC Communication Services — Communication Services Select Sector (XLC)
direction as an advertising/media spending signal.
Includes META, GOOG, NFLX, DIS. When XLC rises, ad spending is healthy
= consumer economy strong = growth. Combined with Wikipedia "advertising" attention.
"""
import pandas as pd
import numpy as np

STRATEGY = {
    "name": "XLC Comms Signal",
    "hypothesis": "Communication services (XLC) = advertising and media spending. Rising XLC = advertisers spending = consumer demand healthy = growth cycle. Falling XLC = ad budget cuts = recession signal = safety. Ad spending is one of the first things companies cut.",
    "universe": ["XLC", "SOXX", "GDX", "SLV", "XLF"],
    "entry": "XLC 7d momentum positive: buy SOXX/XLF. XLC negative: buy GDX/SLV.",
    "exit": "Rotate every 5 trading days",
    "position_size": "100% in best 5d momentum candidate per regime",
    "eccentricity": "Advertising sector as a recession early warning — when META and Google revenues slow, the entire consumer economy is about to follow.",
}
...
def run(data_fetcher, portfolio, start_date, end_date):
    ...
    for ticker in STRATEGY["universe"]:
        try:
            ...
            continue
    if "XLC" not in prices or len(prices) < 4:
        return
    try:
        wiki = data_fetcher.get_wikipedia_pageviews("Advertising")

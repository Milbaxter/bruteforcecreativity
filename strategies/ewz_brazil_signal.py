"""
EWZ Brazil Signal — iShares MSCI Brazil ETF direction as a commodity
exporter economy signal. Brazil is a major exporter of iron ore, soybeans,
oil, and coffee. EWZ rising = commodity export demand healthy.
Combined with Wikipedia "economy_of_Brazil" attention.
"""
import pandas as pd
import numpy as np

STRATEGY = {
    "name": "EWZ Brazil Signal",
    "hypothesis": "Brazil (EWZ) is a commodity superpower. When EWZ rises, global demand for iron ore, soybeans, oil, and agricultural commodities is healthy = growth. When EWZ falls, commodity demand is weakening = safety mode.",
    "universe": ["EWZ", "SOXX", "GDX", "SLV", "XLF"],
    "entry": "EWZ 7d momentum positive: buy SOXX/XLF. EWZ negative: buy GDX/SLV.",
    "exit": "Rotate every 5 trading days",
    "position_size": "100% in best 5d momentum candidate per regime",
    "eccentricity": "Brazilian equities as global commodity demand proxy — EWZ captures the commodity exporter perspective that US-focused signals miss.",
}
...
def run(data_fetcher, portfolio, start_date, end_date):
    ...
    for ticker in STRATEGY["universe"]:
        try:
            ...
            continue
    if "EWZ" not in prices or len(prices) < 4:
        return
    try:
        wiki = data_fetcher.get_wikipedia_pageviews("Economy_of_Brazil")

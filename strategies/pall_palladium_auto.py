"""
PALL Palladium Auto — Aberdeen Palladium ETF (PALL) direction as auto
industry demand signal. Palladium is used in catalytic converters.
Rising PALL = auto production expanding = growth. Falling = slowing.
Combined with Wikipedia "automobile_industry" attention.
"""
import pandas as pd
import numpy as np

STRATEGY = {
    "name": "PALL Palladium Auto",
    "hypothesis": "Palladium demand is driven primarily by automotive catalytic converters. Rising PALL = auto production expanding = manufacturing healthy = growth. Falling PALL = auto production slowing = industrial slowdown.",
    "universe": ["PALL", "SOXX", "GDX", "SLV", "XLF"],
    "entry": "PALL 7d momentum positive: buy SOXX/XLF. PALL negative: buy GDX/SLV.",
    "exit": "Rotate every 5 trading days",
    "position_size": "100% in best 5d momentum candidate per regime",
    "eccentricity": "Palladium as an auto industry barometer for sector rotation — a niche commodity that reveals manufacturing demand.",
}
...
def run(data_fetcher, portfolio, start_date, end_date):
    ...
    for ticker in STRATEGY["universe"]:
        try:
            ...
            continue
    if "PALL" not in prices or len(prices) < 4:
        return
    try:
        wiki = data_fetcher.get_wikipedia_pageviews("Automotive_industry")
